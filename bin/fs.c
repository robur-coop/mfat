...
CAMLprim value mkfs_getpagesize(__unit()) { return (Val_long(getpagesize())); }

CAMLprim value mkfs_pread(value vfd, value vbuf, value voff, value vlen,
                          value vat) {
...
CAMLprim value mkfs_pwrite(value vfd, value vbuf, value voff, value vlen,
                          value vat) {
  CAMLparam5(vfd, vbuf, voff, vlen, vat);
  intnat off, len, at, ret;
  int fd;
